...
- **ドラッグ&ドロップで完結** — 複数枚まとめて処理可能（JPEG / PNG / WebP / HEIC）
- **iPhone HEIC対応** — 変換不要でそのままドロップOK（`heic-to` で自動JPEG化）
- **YOLOv11n-face + onnxruntime-web** — 群衆写真の小さい顔にも強く、誤検出も少ない
- **タイル分割検出 + NMS** — フル画像 + 重なりあり分割タイルで走らせ、遠方の小さい顔まで拾う
...
- [onnxruntime-web](https://www.npmjs.com/package/onnxruntime-web) — ブラウザ内ONNX推論
- [YOLOv11n-face ONNX](https://huggingface.co/AdamCodd/YOLOv11n-face-detection) — 顔検出モデル（Apache-2.0、`public/models/yolov11n-face.onnx` に同梱、約10MB）
- [heic-to](https://www.npmjs.com/package/heic-to) — HEIC → JPEG変換（ブラウザ内）
- [jszip](https://stuk.github.io/jszip/) — ZIP生成
- [file-saver](https://github.com/eligrey/FileSaver.js) — 保存ダイアログ

## 制限と注意

- 顔検出は `YOLOv11n-face` モデル（WIDER FACEで学習）を使用しており、群衆写真に強い反面、横向き・ごく小さい顔は取りこぼすことがあります。タイル分割で緩和していますが、**最終的にはBefore/Afterで必ず目視確認してください**
- ガウシアンぼかしは完全に不可逆ではありません。**強めの半径**を適用していますが、極端に小さいBBoxでは情報量が残り得るため、確実にプライバシーを守りたい用途ではプレビュー確認を推奨します
- ONNXモデルはリポジトリに同梱。onnxruntime-web のWASMはCDN（`jsdelivr`）から取得するため初回のみネットワークが必要。完全オフライン運用したい場合はWASMをローカル配置してください
